...
# !!! ВПИШИТЕ СЮДА ID АДМИНИСТРАТОРОВ (через запятую) !!!
...
async def cmd_admin_add_stars(message: types.Message, command: CommandObject):
    ...
    Использование: /add_stars <id или @username> <количество>
    """
    if message.from_user.id not in ADMIN_IDS:
        return # Игнорируем не админов

    if not command.args:
        await message.answer("⚠️ Использование: <code>/add_stars <id или @username> <количество></code>", parse_mode="HTML")
        return
    
    parts = command.args.split()
    if len(parts) != 2:
        await message.answer("⚠️ Ошибка. Пример: <code>/add_stars @durov 100</code>", parse_mode="HTML")
        return
    ...
    # Определяем, ввел админ ID или Username
    target_id = None
    if target_input.isdigit():
        ...
        if not user_data:
             await message.answer(f"⚠️ Пользователь с ID <code>{target_id}</code> не найден в базе.", parse_mode="HTML")
             return
    else:
        # Пробуем найти по юзернейму
        target_id = await db.get_user_id_by_username(target_input)
        if not target_id:
            await message.answer(f"⚠️ Пользователь <b>{target_input}</b> не найден в базе.", parse_mode="HTML")
            return
    ...
    try:
        await message.bot.send_message(
            ...
            f"Вам начислено: <b>{amount} ⭐️</b>\n"
            f"Текущий баланс: {await db.get_balance(target_id)} ⭐️",
            parse_mode="HTML"
        )
    except:
        await message.answer("ℹ️ Пользователь не получил уведомление (бот заблокирован или ошибка).")
...
async def cmd_admin_users(message: types.Message):
    ...
    # Формируем список
    for user in users:
        u_name = f"@{user['username']}" if user['username'] else "Без юзернейма"
        line = f"ID: <code>{user['user_id']}</code> | {u_name} | <b>{user['balance']} ⭐️</b>\n"
        
        # Если сообщение становится слишком длинным, отправляем кусками
...
async def cmd_start(message: types.Message, command: CommandObject):
    ...
    if is_new and referrer_id:
        try:
            ...
            # Тут можно добавить логику уведомления
            pass 
        except:
            pass
